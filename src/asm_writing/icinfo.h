...
public:
    ...
    void commit(CommitHook* hook, std::vector<void*> gc_references, std::vector<std::pair<uint64_t, std::vector<Location>>> decref_info);
    void abort();
...
public:
    ...
    std::vector<Location> decref_info_add;
private:
    // for ICSlotRewrite:
