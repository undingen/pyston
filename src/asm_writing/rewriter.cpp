...
void RewriterVar::addGuard(uint64_t val) {
    ...
    rewriter->addAction(RewriterAction::createGuard(this, val_var), { this, val_var }, ActionType::GUARD);
}
...
void RewriterVar::addAttrGuard(int offset, uint64_t val, bool negate) {
    ...
    //                    ActionType::GUARD);
    rewriter->addAction(RewriterAction::createAttrGuard(this, offset, val_var, negate), { this, val_var },
                        ActionType::GUARD);
}
...
RewriterVar* RewriterVar::getAttr(int offset, Location dest, assembler::MovType type) {
    ...
    // ActionType::NORMAL);
    rewriter->addAction(RewriterAction::createGetAttr(result, this, offset, dest, type), { this }, ActionType::NORMAL);
    return result;
}
...
void Rewriter::commit() {
    ...
    for (int i = 0; i < actions.size(); i++) {
        ...
        if (a.action && a.op == RewriterAction::Generic)
            a.action();
        else {
            if (a.op == RewriterAction::Guard) {
                _addGuard(a.args.guard.var, a.args.guard.val_constant);
            } else if (a.op == RewriterAction::AttrGuard) {
                _addAttrGuard(a.args.attr_guard.var, a.args.attr_guard.offset, a.args.attr_guard.val,
                              a.args.attr_guard.negate);
            } else if (a.op == RewriterAction::GetAttr) {
                _getAttr(a.args.get_attr.result, a.args.get_attr.ptr, a.args.get_attr.offset,
...
void Rewriter::commitReturning(RewriterVar* var) {
    ...
    addAction(RewriterAction::createCommit(var), { var }, ActionType::NORMAL);
    commit();
}
