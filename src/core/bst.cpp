...
bool PrintVisitor::visit_callattr(BST_CallAttr* node) {
    ...
    stream << ".";
    stream << constant_vregs.getInternedString(node->index_attr).s();
    stream << "(";
...
bool PrintVisitor::visit_classdef(BST_ClassDef* node) {
    ...
    stream << "class " << constant_vregs.getInternedString(node->index_name).s() << "(";
...
bool PrintVisitor::visit_deletename(BST_DeleteName* node) {
    ...
    stream << constant_vregs.getInternedString(node->index_id).s();
    return true;
}
...
bool PrintVisitor::visit_functiondef(BST_FunctionDef* node) {
    ...
    if (node->index_name != VREG_UNDEFINED)
        stream << constant_vregs.getInternedString(node->index_name).s();
    else
        stream << "<lambda>";
    stream << "(";
...
bool PrintVisitor::visit_loadattr(BST_LoadAttr* node) {
    visit_vreg(&node->vreg_dst, true);
    visit_vreg(&node->vreg_value);
    stream << (node->clsonly ? ':' : '.') << constant_vregs.getInternedString(node->index_attr).s();
    return true;
}
