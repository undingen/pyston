...
private:
    ...
    CompilerVariable* evalVReg(int vreg, bool is_kill = true) {
        ...
        if (vreg < 0) {
            Box* o = irstate->getSourceInfo()->constant_vregs.getConstant(vreg);
            if (o->cls == int_cls) {
    ...
    CompilerVariable* evalSliceExprPost(AstType* node, const UnwindInfo& unw_info, CompilerVariable* rtn) {
        ...
        if (speculated_class != NULL) {
            ...
            if (VERBOSITY("irgen") >= 2) {
                ...
                fflush(stdout);
                print_bst(node, irstate->getSourceInfo()->constant_vregs);
                llvm::outs().flush();
                printf("\n");
            }
            ...
            if (rtn->canConvertTo(speculated_type)) {
                ...
                irstate->getSourceInfo()->cfg->print(irstate->getSourceInfo()->constant_vregs);
            }
