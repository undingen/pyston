...
const ConstantVRegInfo& IRGenState::getConstantVRegInfo() {
    return getCode()->constant_vregs;
}
...
private:
    ...
    CompilerVariable* evalCall(BST_Call* node, const UnwindInfo& unw_info) {
        ...
        if (node->type == BST_TYPE::CallAttr) {
            ...
            attr = irstate->getConstantVRegInfo().getInternedString(attr_ast->index_attr);
        ...
        if (node->num_keywords)
            keyword_names = irstate->getConstantVRegInfo().getKeywordNames(node->index_keyword_names);
    ...
    ConcreteCompilerVariable* _getGlobal(BST_LoadName* node, const UnwindInfo& unw_info) {
        InternedString id = irstate->getConstantVRegInfo().getInternedString(node->index_id);
        if (id.s() == "None")
        ...
        if (do_patchpoint) {
            ...
            llvm_args.push_back(
                emitter.setType(embedRelocatablePtr(id.getBox(), g.llvm_boxedstring_type_ptr), RefType::BORROWED));
    ...
    CompilerVariable* evalVReg(int vreg, bool is_kill = true) {
        ...
        if (vreg < 0) {
            Box* o = irstate->getConstantVRegInfo().getConstant(vreg);
            if (o->cls == int_cls) {
    ...
    CompilerVariable* evalLoadName(BST_LoadName* node, const UnwindInfo& unw_info) {
        ...
        if (vst == ScopeInfo::VarScopeType::GLOBAL) {
            ...
            llvm::CallSite call = emitter.createCall(unw_info, g.funcs.assertFailDerefNameDefined,
                                                     embedRelocatablePtr(id.c_str(), g.i8_ptr));
            ...
            llvm::Value* attr = embedRelocatablePtr(id.getBox(), g.llvm_boxedstring_type_ptr);
            ...
            if (!symbol_table[node->vreg]) {
                ...
                llvm::CallSite call = emitter.createCall(
                    ...
                    { getConstantInt(0, g.i1), embedRelocatablePtr(id.c_str(), g.i8_ptr),
            ...
            if (is_defined_var) {
                emitter.createCall(
                    unw_info, g.funcs.assertNameDefined,
                    { i1FromLLVMBool(emitter, is_defined_var), embedRelocatablePtr(id.c_str(), g.i8_ptr),
                      emitter.setType(embedRelocatablePtr(UnboundLocalError, g.llvm_class_type_ptr), RefType::BORROWED),
                      getConstantInt(true, g.i1) });
    ...
    CompilerVariable* evalLoadAttr(BST_LoadAttr* node, const UnwindInfo& unw_info) {
        CompilerVariable* value = evalVReg(node->vreg_value);
        InternedString attr = irstate->getConstantVRegInfo().getInternedString(node->index_attr);
        CompilerVariable* rtn = value->getattr(emitter, getOpInfoForNode(node, unw_info), attr.getBox(), node->clsonly);
        return rtn;
    }
    ...
    CompilerVariable* evalMakeClass(BST_MakeClass* mkclass, const UnwindInfo& unw_info) {
        auto class_entry = irstate->getConstantVRegInfo().getFuncOrClass(mkclass->index_class_def);
        ...
        InternedString name = irstate->getConstantVRegInfo().getInternedString(node->index_name);
    ...
    CompilerVariable* evalMakeFunction(BST_MakeFunction* mkfn, const UnwindInfo& unw_info) {
        auto func_entry = irstate->getConstantVRegInfo().getFuncOrClass(mkfn->index_function_def);
    ...
    CompilerVariable* evalSliceExprPost(AstType* node, const UnwindInfo& unw_info, CompilerVariable* rtn) {
        ...
        if (speculated_class != NULL) {
            ...
            if (VERBOSITY("irgen") >= 2) {
                ...
                fflush(stdout);
                print_bst(node, irstate->getConstantVRegInfo());
                llvm::outs().flush();
                printf("\n");
            }
            ...
            if (rtn->canConvertTo(speculated_type)) {
                ...
                irstate->getSourceInfo()->cfg->print(irstate->getConstantVRegInfo());
            }
    ...
    void doStoreName(BST_StoreName* node, const UnwindInfo& unw_info) {
        InternedString name = irstate->getConstantVRegInfo().getInternedString(node->index_id);
    ...
    void doStoreAttr(BST_StoreAttr* target, const UnwindInfo& unw_info) {
        ...
        InternedString attr = irstate->getConstantVRegInfo().getInternedString(target->index_attr);
    ...
    void _doDelAttr(BST_DeleteAttr* node, const UnwindInfo& unw_info) {
        InternedString attr = irstate->getConstantVRegInfo().getInternedString(node->index_attr);
        CompilerVariable* value = evalVReg(node->vreg_value);
        value->delattr(emitter, getEmptyOpInfo(unw_info), attr.getBox());
    }

    void _doDelName(BST_DeleteName* target, const UnwindInfo& unw_info) {
        InternedString id = irstate->getConstantVRegInfo().getInternedString(target->index_id);
        ...
        if (vst == ScopeInfo::VarScopeType::GLOBAL) {
            ...
            emitter.createCall2(
                unw_info, g.funcs.delGlobal, irstate->getGlobals(),
                emitter.setType(embedRelocatablePtr(id.getBox(), g.llvm_boxedstring_type_ptr), RefType::BORROWED));
            return;
        }
        ...
        if (!symbol_table[target->vreg]) {
            llvm::CallSite call = emitter.createCall(
                unw_info, g.funcs.assertNameDefined,
                { getConstantInt(0, g.i1), embedConstantPtr(id.c_str(), g.i8_ptr),
        ...
        if (is_defined_var) {
            emitter.createCall(
                unw_info, g.funcs.assertNameDefined,
                { i1FromLLVMBool(emitter, is_defined_var), embedConstantPtr(id.c_str(), g.i8_ptr),
                  emitter.setType(embedRelocatablePtr(NameError, g.llvm_class_type_ptr), RefType::BORROWED),
                  getConstantInt(true /*local_error_msg*/, g.i1) });
