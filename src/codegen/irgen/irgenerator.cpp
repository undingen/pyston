...
private:
    ...
    void doStoreAttr(BST_StoreAttr* target, const UnwindInfo& unw_info) {
        ...
        InternedString attr = irstate->getCodeConstants().getInternedString(target->index_attr);
...
#endif
            if (stmt->isInvoke()) {
                doStmt(stmt, UnwindInfo(irstate->getCode(), stmt, entry_blocks[stmt->getExcBlock()]));

                assert(state == RUNNING || state == DEAD);
                if (state == RUNNING) {
                    emitter.getBuilder()->CreateBr(entry_blocks[stmt->getDefBlock()]);
                    endBlock(FINISHED);
                }
            } else
