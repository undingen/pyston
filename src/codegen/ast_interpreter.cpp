...
private:
    ...
    void abortJITing();
    void finishJITing(CFGBlock* continue_block = nullptr);
...
void ASTInterpreter::startJITing(CFGBlock* block, int jump_offset) {
    ...
    JitCodeBlock* code_block = nullptr;
...
void ASTInterpreter::doStore(AST_expr* node, Value value) {
    if (node->type == AST_TYPE::Name) {
        ...
        for (AST_expr* e : tuple->elts) {
            doStore(e, Value(array[i], jit ? array_var->getAttr(i * 8) : NULL));
            ++i;
        }
...
Value ASTInterpreter::visit_unaryop(AST_UnaryOp* node) {
    ...
    if (node->op_type == AST_TYPE::Not) {
        Value v;
        if (jit)
            v.var = jit->emitNotNonzero(operand);
        ...
        Value v;
        if (jit)
            v.var = jit->emitUnaryop(operand, node->op_type);
...
Value ASTInterpreter::visit_clsAttribute(AST_ClsAttribute* node) {
    ...
    return Value(getclsattr(obj.o, attr), jit ? jit->emitGetClsAttr(obj, attr) : nullptr);
}
...
Value ASTInterpreter::visit_langPrimitive(AST_LangPrimitive* node) {
    ...
    if (node->opcode == AST_LangPrimitive::GET_ITER) {
        ...
        v = Value(BoxedTuple::create({ type, value, traceback }), jit ? jit->emitLandingpad() : nullptr);
...
Value ASTInterpreter::visit_call(AST_Call* node) {
    ...
    if (is_callattr) {
        ...
        if (jit)
            v.var = jit->emitCallattr(func, attr.getBox(), flags, argspec, argsValues, keyword_names);
...
Value ASTInterpreter::visit_num(AST_Num* node) {
    ...
    if (node->num_type == AST_Num::INT) {
        ...
        RELEASE_ASSERT(0, "not implemented");
    return Value(o, jit ? jit->imm(o) : nullptr);
}
