...
void ASTInterpreter::doStore(int vreg, STOLEN(Value) value) {
    ...
    if (jit) {
        bool is_live = source_info->getLiveness(getConstantVRegInfo())->isLiveAtEnd(vreg, current_block);
        if (is_live)
...
Box* ASTInterpreter::doOSR(BST_Jump* node) {
    ...
    LivenessAnalysis* liveness = source_info->getLiveness(getConstantVRegInfo());
    std::unique_ptr<PhiAnalysis> phis
        = computeRequiredPhis(getCode()->param_names, source_info->cfg, liveness, getConstantVRegInfo());
...
Value ASTInterpreter::visit_makeFunction(BST_MakeFunction* mkfn) {
    auto func_entry = getCode()->constant_vregs.getFuncOrClass(mkfn->index_function_def);
...
Value ASTInterpreter::visit_makeClass(BST_MakeClass* mkclass) {
    ...
    auto class_entry = getCode()->constant_vregs.getFuncOrClass(mkclass->index_class_def);
    ...
    InternedString name = getConstantVRegInfo().getInternedString(node->index_name);
...
void ASTInterpreter::visit_deleteattr(BST_DeleteAttr* node) {
    ...
    BoxedString* str = getConstantVRegInfo().getInternedString(node->index_attr).getBox();
    if (jit)
...
void ASTInterpreter::visit_deletename(BST_DeleteName* target) {
    ...
    InternedString id = getConstantVRegInfo().getInternedString(target->index_id);
...
Value ASTInterpreter::visit_call(BST_Call* node) {
    ...
    if (node->type == BST_TYPE::CallAttr) {
        ...
        attr = getConstantVRegInfo().getInternedString(attr_ast->index_attr);
    ...
    if (node->num_keywords)
        keyword_names = getConstantVRegInfo().getKeywordNames(node->index_keyword_names);
...
Value ASTInterpreter::getVReg(int vreg, bool is_kill) {
    ...
    if (jit) {
        ...
        if (is_kill) {
            ...
            is_live = source_info->getLiveness(getConstantVRegInfo())->isLiveAtEnd(vreg, current_block);
        }
...
Value ASTInterpreter::visit_loadname(BST_LoadName* node) {
    assert(node->lookup_type != ScopeInfo::VarScopeType::UNKNOWN);
    InternedString id = getConstantVRegInfo().getInternedString(node->index_id);
        ...
        case ScopeInfo::VarScopeType::CLOSURE: {
            ...
            if (jit) {
                ...
                if (node->lookup_type == ScopeInfo::VarScopeType::FAST) {
                    assert(node->vreg >= 0);
                    is_live = source_info->getLiveness(getConstantVRegInfo())->isLiveAtEnd(node->vreg, current_block);
                }
...
Value ASTInterpreter::visit_loadattr(BST_LoadAttr* node) {
    ...
    BoxedString* attr = getConstantVRegInfo().getInternedString(node->index_attr).getBox();
    Value r;
    if (node->clsonly)
...
void ASTInterpreter::visit_storename(BST_StoreName* node) {
    ...
    InternedString name = getConstantVRegInfo().getInternedString(node->index_id);
    ScopeInfo::VarScopeType vst = node->lookup_type;
    if (vst == ScopeInfo::VarScopeType::GLOBAL) {
        if (jit)
...
void ASTInterpreter::visit_storeattr(BST_StoreAttr* node) {
    ...
    InternedString attr = getConstantVRegInfo().getInternedString(node->index_attr);
    if (jit) {
...
Box* ASTInterpreterJitInterface::derefHelper(void* _interpreter, BST_LoadName* node) {
    ...
    if (val == NULL) {
        InternedString id = interpreter->getConstantVRegInfo().getInternedString(node->index_id);
