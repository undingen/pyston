...
public:
    ...
    const llvm::DenseMap<InternedString, DefaultedInt<-1>>& getSymVRegMap() {
...
void ASTInterpreter::doStore(AST_Name* node, STOLEN(Value) value) {
    ...
    if (vst == ScopeInfo::VarScopeType::GLOBAL) {
        ...
        if (jit) {
            bool is_live = true;
            if (!closure)
                is_live = getSourceInfo()->getLiveness()->isLiveAtEnd(node->vreg, current_block);
            if (is_live)
...
Box* ASTInterpreter::doOSR(AST_Jump* node) {
    ...
    int num_vregs = getSourceInfo()->cfg->getVRegInfo().getTotalNumOfVRegs();
