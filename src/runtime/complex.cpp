...
template <ExceptionStyle S> static Box* try_special_method(Box* self) noexcept(S == CAPI) {
    ...
    if (PyObject_HasAttr((PyObject*)self, float_str)) {
        Box* r_f = callattrInternal<S, false>(self, float_str, CLASS_ONLY, NULL, ArgPassSpec(0), NULL, NULL, NULL, NULL,
                                              NULL);
    ...
    if (PyObject_HasAttr((PyObject*)self, complex_str)) {
        Box* r = callattrInternal<S, false>(self, complex_str, CLASS_OR_INST, NULL, ArgPassSpec(0), NULL, NULL, NULL,
                                            NULL, NULL);
        if (!r) {
            if (S == CAPI) {
                if (!PyErr_Occurred())
