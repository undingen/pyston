...
public:
    ...
    bool operator()(Box* lhs, Box* rhs) {
        Box* r = runtimeCallInternal<CXX, false>(cmp, NULL, ArgPassSpec(2), lhs, rhs, NULL, NULL, NULL);
        if (!PyInt_Check(r))
...
Box* _listCmp(BoxedList* lhs, BoxedList* rhs, AST_TYPE::AST_TYPE op_type) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (op_type == AST_TYPE::Eq) {
            ...
            Box* r = compareInternal<false>(lhs->elts->elts[i], rhs->elts->elts[i], op_type, NULL);
            return r;
        }
    }
